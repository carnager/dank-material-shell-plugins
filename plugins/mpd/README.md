...
The widget now calls an external `mpdwatch` binary directly. Build it from the top-level `tools/mpdwatch/` project:
...
- The runtime dependency is now the external `mpdwatch` binary from [`../../tools/mpdwatch`](../../tools/mpdwatch).
